...
data1 = np.empty([100, 2*parameter_num+1])
data2 = np.empty([100, 2*parameter_num+1])
data3 = np.empty([100, 2*parameter_num+1])

for i in range(0, 100):
	y, sr = librosa.load('D:/dataset/train/positive/' + str(i) + '/audio.wav', sr=16000)#initial sr=16000
...
for i in range(0, 100):
	y, sr = librosa.load('D:/dataset/train/negative/' + str(i) + '/audio.wav', sr=16000)
	mfccs = librosa.feature.mfcc(y = y, sr = sr, n_mfcc = parameter_num)
	for j in range(0, parameter_num):
...
train_data, test_data, train_label, test_label = train_test_split(x, y, random_state = 2, train_size = 0.7, test_size = 0.3) #sklearn.model_selection.
